...
await fetch(webhook, {
    ...
    body: JSON.stringify({
        embeds: [{
            ...
            color: 39902,
            description: `${codeblock}npm install ${pkg.name}@${pkg.version}${codeblock}${codeblock}bun add ${pkg.name}@${pkg.version}${codeblock}${codeblock}yarn add ${pkg.name}@${pkg.version}${codeblock}\n**report cockroaches (& other bugs) in** <#1342571945123774527>`
        }]
    }),
    headers: {
